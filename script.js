...
function toggleCard(e) {
	if (e.target == addedBtn || e.target == cardSection || e.target == saveBtn) {
...
// Book Constructor
class Book {
	constructor(cover, name, author, pages, tag) {
		this.cover = cover;
		this.name = name;
		this.author = author;
		this.pages = pages;
...
function displayBooks(bookArray) {
	...
	bookArray.forEach((element) => {
		bookShelf.innerHTML += `<div data-index="${myLibrary.length - 1}" class="book">
...
bookShelf.addEventListener("click", (e) => {
	if (e.target.parentElement.parentElement.className == "book" && e.target.id == "status") {
